function [H_r,Ur,Z,G] = invrG(rho,c,k,r_ms,e_n,CP,r_lh,n_ma,plot)
    n = size(e_n,1);
    m = size(r_ms,1);
    I_n1s = gpuArray.zeros(m,3,n);
    I_n1s(sub2ind([m 3 n],reshape(e_n,[],1),reshape(gpuArray.ones(n,3).*gpuArray(1:3),[],1),repmat(gpuArray(1:n)',3,1))) = 1;
    I_n1 = gpuArray.ones(m,1)-sum(I_n1s,2);
    I_n2 = permute(I_n1s,[2,1,3]);
    load('IPF16-40.mat','IPF','IPFs')
    Xi = gpuArray([IPFs(:,1,:) IPFs(:,2,:) 1-IPFs(:,1,:)-IPFs(:,2,:)]);
    w = gpuArray(IPFs(:,3,1)');
    ettaller = gpuArray.ones(size(IPFs,1),1);
    nuller = gpuArray.zeros(size(IPFs,1),1);
    ...
    Xi = gpuArray([IPF(:,1) IPF(:,2) 1-IPF(:,1)-IPF(:,2)]);
    w = gpuArray(IPF(:,3)');
    ettaller = gpuArray.ones(size(IPF,1),1);
    nuller = gpuArray.zeros(size(IPF,1),1);
    dNx = [ettaller nuller -ettaller];
    dNy = [nuller ettaller -ettaller];
    dxde1 = pagemtimes(dNx,exv(:,1,:));
    dyde1 = pagemtimes(dNx,exv(:,2,:));
