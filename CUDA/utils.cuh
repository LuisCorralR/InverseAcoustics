...
void line_source(double *r_ms, double *r_lh, int *e_n, cuDoubleComplex **p_s, cuDoubleComplex **p_h, cuDoubleComplex **v_s, double k, \
                 int n_ls, int m, int l, int n)
{
    int i, j; // Iterators are reused
    double c = C_AIR, rho = RHO_AIR;
    
    double *nvect = (double*)malloc(m * 3 * sizeof(double));
    memset(nvect, 0.0, m * 3 * sizeof(double));
    for (i = 0; i < n; i++){
        double dxde1, dyde1, dzde1, dxde2, dyde2, dzde2, nx, ny, nz;
        ...
        for (j = 0; j < 3; j++){
            nvect[e_n[i+j*n]] += nx;
            nvect[e_n[i+j*n]+m] += ny;
            nvect[e_n[i+j*n]+2*m] += nz;
        }
    }
    for (i = 0; i < m; i++)
    {
        double d = sqrt(pow(nvect[i], 2.0) + pow(nvect[i+m], 2.0) + pow(nvect[i+2*m], 2.0));
        nvect[i] /= d;
        nvect[i+m] /= d;
        nvect[i+2*m] /= d; 
    }
    ...
    *p_s = (cuDoubleComplex*)malloc(m * sizeof(cuDoubleComplex));
    *v_s = (cuDoubleComplex*)malloc(m * sizeof(cuDoubleComplex));
    thrust::complex<double> tpt, tp_nt, tp_et, dtpt, dtp_nt, dtp_et, p_st, v_st;
    thrust::complex<double> I(0.0, 1.0);
    double drn, r;
    for (i = 0; i < m; ++i)
    {
        ...
        {
            r = sqrt(pow(r_ms[i], 2) + \
                     pow(r_ms[i+m]-((-0.5/2)+((0.5/(n_ls-1))*j)), 2) + \
                     pow(r_ms[i+2*m], 2));
            drn = ((nvect[i] * r_ms[i]) + (nvect[i+m] * (r_ms[i+m]-((-0.5/2)+((0.5/(n_ls-1))*j)))) +
                   (nvect[i+2*m] * r_ms[i+2*m])) / r;
            if (j == 0)
            {
                tp_nt = exp(I * k * r) / r;
                dtp_nt = (I * k * r - 1) * (exp(I * k * r) / pow(r, 2.0)) * drn;
            } else
            {
                tp_et = exp(I * k * r) / r;
                dtp_et = (I * k * r - 1) * (exp(I * k * r) / pow(r, 2.0)) * drn;
                tpt += tp_nt + tp_et;
                dtpt += dtp_nt + dtp_et;
                tp_nt = tp_et;
                dtp_nt = dtp_et;
        ...
        p_st = I * ((rho * c * 100 * k * 0.01) / 2) * ((0.5 / (2 * (n_ls - 1))) * tpt);
        (*p_s)[i] = make_cuDoubleComplex(p_st.real(), p_st.imag());
        v_st = ((100 * 0.01) / 2) * ((0.5 / (2 * (n_ls - 1))) * dtpt);
        (*v_s)[i] = make_cuDoubleComplex(v_st.real(), v_st.imag());
    }
    free(nvect);
    ...
    *p_h = (cuDoubleComplex*)malloc(l * sizeof(cuDoubleComplex));
    ...
    for (i = 0; i < m; ++i)
    {
        ...
        {
            r = sqrt(pow(r_lh[i], 2) + \
                     pow(r_lh[i+m]-((-0.5/2)+((0.5/(n_ls-1))*j)), 2) + \
                     pow(r_lh[i+2*m], 2));
            if (j == 0)
            {
                tp_nt = exp(thrust::complex<double>(0.0, 1.0) * k * r) / r;
            } else
            {
                tp_et = exp(thrust::complex<double>(0.0, 1.0) * k * r) / r;
                tpt += tp_nt + tp_et;
                tp_nt = tp_et;
            }
        }
        p_ht = thrust::complex<double>(0.0, 1.0)  * ((rho * c * 100 * k * 0.01) / 2) * ((0.5 / (2 * (n_ls - 1))) * tpt);
        (*p_h)[i] = make_cuDoubleComplex(p_ht.real(), p_ht.imag());
    }
}
