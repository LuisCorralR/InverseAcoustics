__global__ void invrg_singular_surface(double *r_ms, int *e_n, double *ipfs, int ipfsl, cuDoubleComplex *atxs0, double *ct, double k,\
...
__global__ void invrg_nonsingular_surface(double *r_ms, int *e_n, double *ipf, int ipfl, cuDoubleComplex *atx0, double *ct, double k,\
...
__global__ void invrg_update_s(cuDoubleComplex *ae, cuDoubleComplex *be, double *ce, int *e_n, int m, int n, int i, int j, \
...
__global__ void invrg_surface_kernel_s(double *r_ms, int *e_n, double *ipfs, int ipfsl, cuDoubleComplex *ae, cuDoubleComplex *be,\
                                       double *ce, double k, int m, int n, int i, int j)
{
    ...
    invrg_singular_surface<<<grid,BLOCK>>>(r_ms, e_n, ipfs, ipfsl, axbx, ct, k, i, j, m, n);
    invrg_update_s<<<1,1>>>(ae, be, ce, e_n, m, n, i, j, axbx, ct);
    free(axbx);
    free(ct);
}

__global__ void invrg_update_ns(cuDoubleComplex *ae, cuDoubleComplex *be, double *ce, int m, int n, int i, int o, cuDoubleComplex *axbx, \
                                double *ct)
{
...
__global__ void invrg_surface_kernel_ns(double *r_ms, int *e_n, double *ipf, int ipfl, cuDoubleComplex *ae, cuDoubleComplex *be, \
                                        double *ce, double k, int m, int n, int i, int gr)
{
    ...
    if (o < m)
    {
        if (e_n[i] != o && e_n[i+n] != o && e_n[i+2*n] != o)
        {
            ...
            memset(axbx, 0, 6*sizeof(cuDoubleComplex));
            invrg_nonsingular_surface<<<grid,BLOCK>>>(r_ms, e_n, ipf, ipfl, axbx, ct, k, i, o, m, n);
            __syncthreads();
            invrg_update_ns<<<1,1>>>(ae, be, ce, m, n, i, o, axbx, ct);
            free(axbx);
            free(ct);
        }
    }
...
__global__ void invrg_updatesurface_kernel(cuDoubleComplex *ae, cuDoubleComplex *be, double *ce, cuDoubleComplex *ass, \
...
__global__ void invrg_nonsingular_field(double *r_ms, double *r_lh, int *e_n, double *ipf, int ipfl, cuDoubleComplex *atx0, double k,\
...
__global__ void invrg_update_fns(cuDoubleComplex *ahs, cuDoubleComplex *bhs, int *e_n, int l, int n, int i, int j, cuDoubleComplex *axbx)
{
...
__global__ void invrg_field_kernel(double *r_ms, double *r_lh, int *e_n, double *ipf, int ipfl, cuDoubleComplex *ahs, \
                                   cuDoubleComplex *bhs, double k, int l, int m, int n, int j, int gr)
{
    ...
    if (i < l)
    {
        ...
        cuDoubleComplex* axbx = (cuDoubleComplex*)malloc(6*sizeof(cuDoubleComplex));
        memset(axbx, 0, 6*sizeof(cuDoubleComplex));
        invrg_nonsingular_field<<<grid,BLOCK>>>(r_ms, r_lh, e_n, ipf, ipfl, axbx, k, i, j, l, m, n);
        __syncthreads();
        invrg_update_fns<<<1,1>>>(ahs, bhs, e_n, l, n, i, j, axbx);
        free(axbx);
    }
}

__global__ void invrg_update_cns(cuDoubleComplex *ass, cuDoubleComplex *bss, int *e_n, int mt, int n, int m, int i, int j, \
...
__global__ void invrg_chief_kernel(double *r_ms, double *cp, int *e_n, double *ipf, int ipfl, cuDoubleComplex *ass, \
                                   cuDoubleComplex *bss, double k, int n_ch, int m, int n, int mt, int j)
{
    ...
    if (i < n_ch)
    {
        ...
        cuDoubleComplex* axbx = (cuDoubleComplex*)malloc(6*sizeof(cuDoubleComplex));
        memset(axbx, 0, 6*sizeof(cuDoubleComplex));
        invrg_nonsingular_field<<<grid,BLOCK>>>(r_ms, cp, e_n, ipf, ipfl, axbx, k, i, j, n_ch, m, n);
        invrg_update_cns<<<1,1>>>(ass, bss, e_n, mt, n, m, i, j, axbx);
        free(axbx);
    }
}

void invrgpu(int m, int l, int n, double *r_msg, int *e_ng, double *r_lhg, cuDoubleComplex **me, cuDoubleComplex **he, cuDoubleComplex **qe, double k, int n_ch, double *cp, int n_rm)
{
    // IPF reading
    int i, j;
    ...
    // Main kernels
    int mt = m + n_ch;
    cuDoubleComplex *ae, *be, *ass, *bss, *gm, *rm, *ahs, *bhs, *z, *partial, *partial2;
    ...
    CUDA_CHECK(cudaMallocAsync((void **)&gm, sizeof(cuDoubleComplex) * m * m, surface));
    ...
    CUDA_CHECK(cudaMemsetAsync(gm, 0.0, m*m*sizeof(cuDoubleComplex), surface));
    ...
    for (i = 0; i < n; i++)
    {
        ...
        invrg_surface_kernel_s<<<1,1,0,surface>>>(r_msg, e_ng, ipfsg, ipfsl, ae, be, ce, k, m, n, i, 0);
        invrg_surface_kernel_s<<<1,1,0,surface>>>(r_msg, e_ng, ipfsg, ipfsl, ae, be, ce, k, m, n, i, 1);
        invrg_surface_kernel_s<<<1,1,0,surface>>>(r_msg, e_ng, ipfsg, ipfsl, ae, be, ce, k, m, n, i, 2);
        grid = (int)ceil((double)m/BLOCK);
        for (j = 0; j < grid; j++){
            invrg_surface_kernel_ns<<<1,BLOCK,0,surface>>>(r_msg, e_ng, ipfg, ipfl, ae, be, ce, k, m, n, i, j);
            CUDA_CHECK(cudaStreamSynchronize(surface));
        }
        invrg_updatesurface_kernel<<<grid,BLOCK,0,surface>>>(ae, be, ce, ass, bss, e_ng, mt, m, n, i);
        grid = (int)ceil((double)l/BLOCK);
        for (j = 0; j < grid; j++){
            invrg_field_kernel<<<1,BLOCK,0,field>>>(r_msg, r_lhg, e_ng, ipfg, ipfl, ahs, bhs, k, l, m, n, i, j);
            CUDA_CHECK(cudaStreamSynchronize(field));
        }
        invrg_chief_kernel<<<1,n_ch,0,surface>>>(r_msg, cpg, e_ng, ipfg, ipfl, ass, bss, k, l, m, n, mt, i);
    }
    CUDA_CHECK(cudaStreamSynchronize(surface));
    CUDA_CHECK(cudaStreamSynchronize(field));
    ...
    free(ipf);
    free(ipfs);
    thrust::complex<double> I(0.0, 1.0), scal;
    scal = I * rho * c * k;
    ...
    CUDA_CHECK(cudaMallocAsync((void **)&rm, sizeof(cuDoubleComplex) * m * m, solver));
    cublasZgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, m, m, m, &alpha, gm, m, z, m, &beta, rm, m);
    CUDA_CHECK(cudaFreeAsync(gm, surface));
    
    // SVD for R
    int *d_info = nullptr;
    int lwork = 0;
    double *sg;
    cuDoubleComplex *ug, *vg;
    double *d_rwork = nullptr;
    cuDoubleComplex *d_svdwork = nullptr;
    CUDA_CHECK(cudaMallocAsync((void **)&ug, sizeof(cuDoubleComplex) * m * m, solver));
    CUDA_CHECK(cudaMallocAsync((void **)&sg, sizeof(double) * m, solver));
    CUDA_CHECK(cudaMallocAsync((void **)&vg, sizeof(cuDoubleComplex) * m * m, solver));
    CUSOLVER_CHECK(cusolverDnZgesvd_bufferSize(handle_s, m, m, &lwork));
    CUDA_CHECK(cudaMallocAsync((void **)&d_svdwork, sizeof(cuDoubleComplex) * lwork, solver));
    CUDA_CHECK(cudaMallocAsync((void **)&d_info, sizeof(int), solver));
    CUSOLVER_CHECK(cusolverDnZgesvd(handle_s, 'S', 'A', m, m, rm, m, sg, ug, m, vg, m, d_svdwork, lwork, d_rwork, d_info));
    CUDA_CHECK(cudaFreeAsync(vg, solver));
    CUDA_CHECK(cudaFreeAsync(sg, solver));
    CUDA_CHECK(cudaFreeAsync(d_info, solver));
    CUDA_CHECK(cudaFreeAsync(d_svdwork, solver));
    cublasZgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, l, n_rm, m, &alpha, partial2, l, ug, m, &beta, partial, l);
    CUDA_CHECK(cudaFreeAsync(partial2, solver));
    cublasZgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, m, n_rm, m, &alpha, z, m, ug, m, &beta, rm, m);
    CUDA_CHECK(cudaFreeAsync(z, solver));
    
    // Final Matrices
    *qe = (cuDoubleComplex*)malloc(m * n_rm * sizeof(cuDoubleComplex));
    CUDA_CHECK(cudaMemcpyAsync(*qe, ug, m*n_rm*sizeof(cuDoubleComplex), cudaMemcpyDeviceToHost, solver));
    CUDA_CHECK(cudaFreeAsync(ug, solver));
    *me = (cuDoubleComplex*)malloc(m * n_rm * sizeof(cuDoubleComplex));
    CUDA_CHECK(cudaMemcpyAsync(*me, rm, m*n_rm*sizeof(cuDoubleComplex), cudaMemcpyDeviceToHost, solver));
    CUDA_CHECK(cudaFreeAsync(rm, solver));
    *he = (cuDoubleComplex*)malloc(l * n_rm * sizeof(cuDoubleComplex));
    CUDA_CHECK(cudaMemcpyAsync(*he, partial, l*n_rm*sizeof(cuDoubleComplex), cudaMemcpyDeviceToHost, solver));
...
extern "C" void invr(int m, int l, int n, double *r_ms, int *e_n, double *r_lh, cuDoubleComplex **m_e, cuDoubleComplex **h, \
                     cuDoubleComplex **q_e, double k, int n_ch, double *cp, int n_am)
{
    ...
    cuDoubleComplex *gm = (cuDoubleComplex*)calloc(m * m, sizeof(cuDoubleComplex));
    double *ce = (double*)calloc(m, sizeof(double));
    double *elnodx = (double*)malloc(3 * sizeof(double));
    double *elnody = (double*)malloc(3 * sizeof(double));
    double *elnodz = (double*)malloc(3 * sizeof(double));
    ...
    for (i = 0; i < n; i++){
        elnodx[0] = r_ms[e_n[i]], elnodx[1] = r_ms[e_n[i+n]], elnodx[2] = r_ms[e_n[i+2*n]];
        elnody[0] = r_ms[e_n[i]+m], elnody[1] = r_ms[e_n[i+n]+m], elnody[2] = r_ms[e_n[i+2*n]+m];
        elnodz[0] = r_ms[e_n[i]+2*m], elnodz[1] = r_ms[e_n[i+n]+2*m], elnodz[2] = r_ms[e_n[i+2*n]+2*m];
        ...
        for (j = 0; j < 3; j++){
            ...
            for(o = 0; o < ipfsl; o++) {
                xq = (ipfs[o+(3*j*ipfsl)] * elnodx[0]) + (ipfs[o+ipfsl+(3*j*ipfsl)] * elnodx[1]) + 
                     ((1.0 - ipfs[o+(3*j*ipfsl)] - ipfs[o+ipfsl+(3*j*ipfsl)]) * elnodx[2]);
                yq = (ipfs[o+(3*j*ipfsl)] * elnody[0]) + (ipfs[o+ipfsl+(3*j*ipfsl)] * elnody[1]) + 
                     ((1.0 - ipfs[o+(3*j*ipfsl)] - ipfs[o+ipfsl+(3*j*ipfsl)]) * elnody[2]);
                zq = (ipfs[o+(3*j*ipfsl)] * elnodz[0]) + (ipfs[o+ipfsl+(3*j*ipfsl)] * elnodz[1]) + 
                     ((1.0 - ipfs[o+(3*j*ipfsl)] - ipfs[o+ipfsl+(3*j*ipfsl)]) * elnodz[2]);
                dxde1 = elnodx[0] - elnodx[2];
                dyde1 = elnody[0] - elnody[2];
                dzde1 = elnodz[0] - elnodz[2];
                dxde2 = elnodx[1] - elnodx[2];
                dyde2 = elnody[1] - elnody[2];
                dzde2 = elnodz[1] - elnodz[2];
                nx = (dyde1 * dzde2) - (dzde1 * dyde2);
                ny = (dzde1 * dxde2) - (dxde1 * dzde2);
                nz = (dxde1 * dyde2) - (dyde1 * dxde2);
                an_xs = sqrt(pow(nx, 2) + pow(ny, 2) + pow(nz, 2));
        ...
        for (o = 0; o < m; o++){
            ...
            if (e_n[i] != o && e_n[i+n] != o && e_n[i+2*n] != o){
                for(q = 0; q < ipfl; q++) {
                    xq = (ipf[q] * elnodx[0]) + (ipf[q+ipfl] * elnodx[1]) + ((1.0 - ipf[q] - ipf[q+ipfl]) * elnodx[2]);
                    yq = (ipf[q] * elnody[0]) + (ipf[q+ipfl] * elnody[1]) + ((1.0 - ipf[q] - ipf[q+ipfl]) * elnody[2]);
                    zq = (ipf[q] * elnodz[0]) + (ipf[q+ipfl] * elnodz[1]) + ((1.0 - ipf[q] - ipf[q+ipfl]) * elnodz[2]);
                    dxde1 = elnodx[0] - elnodx[2];
                    dyde1 = elnody[0] - elnody[2];
                    dzde1 = elnodz[0] - elnodz[2];
                    dxde2 = elnodx[1] - elnodx[2];
                    ...
                    if (o == 0)
                    {
                        gm[e_n[i]+e_n[i]*m] = make_cuDoubleComplex((ipf[q+2*ipfl] * ipf[q] * an_xs) * ipf[q] +
                                                                  cuCreal(gm[e_n[i]+e_n[i]*m]), 0.0);
                        gm[e_n[i]+e_n[i+n]*m] = make_cuDoubleComplex((ipf[q+2*ipfl] * ipf[q] * an_xs) * ipf[q+ipfl] +
                                                                    cuCreal(gm[e_n[i]+e_n[i+n]*m]), 0.0);
                        gm[e_n[i]+e_n[i+2*n]*m] = make_cuDoubleComplex((ipf[q+2*ipfl] * ipf[q] * an_xs) * (1.0 - ipf[q] - ipf[q+ipfl]) + 
                                                                      cuCreal(gm[e_n[i]+e_n[i+2*n]*m]), 0.0);
                        gm[e_n[i+n]+e_n[i]*m] = make_cuDoubleComplex((ipf[q+2*ipfl] * ipf[q+ipfl] * an_xs) * ipf[q] + 
                                                                     cuCreal(gm[e_n[i+n]+e_n[i]*m]), 0.0);
                        gm[e_n[i+n]+e_n[i+n]*m] = make_cuDoubleComplex((ipf[q+2*ipfl] * ipf[q+ipfl] * an_xs) * ipf[q+ipfl] +
                                                                      cuCreal(gm[e_n[i+n]+e_n[i+n]*m]), 0.0);
                        gm[e_n[i+n]+e_n[i+2*n]*m] = make_cuDoubleComplex((ipf[q+2*ipfl] * ipf[q+ipfl] * an_xs) * 
                                                                         (1.0 - ipf[q] - ipf[q+ipfl]) +
                                                                         cuCreal(gm[e_n[i+n]+e_n[i+2*n]*m]), 0.0);
                        gm[e_n[i+2*n]+e_n[i]*m] = make_cuDoubleComplex((ipf[q+2*ipfl] * (1.0 - ipf[q] - ipf[q+ipfl]) * an_xs) * ipf[q] +
                                                                       cuCreal(gm[e_n[i+2*n]+e_n[i]*m]), 0.0);
                        gm[e_n[i+2*n]+e_n[i+n]*m] = make_cuDoubleComplex((ipf[q+2*ipfl] * (1.0 - ipf[q] - ipf[q+ipfl]) * an_xs) *
                                                                         ipf[q+ipfl] + cuCreal(gm[e_n[i+2*n]+e_n[i+n]*m]), 0.0);
                        gm[e_n[i+2*n]+e_n[i+2*n]*m] = make_cuDoubleComplex((ipf[q+2*ipfl] * (1.0 - ipf[q] - ipf[q+ipfl]) * an_xs) *
                                                                           (1.0 - ipf[q] - ipf[q+ipfl]) + 
                                                                           cuCreal(gm[e_n[i+2*n]+e_n[i+2*n]*m]), 0.0);
                    }
                    r = sqrt(pow(xq - r_ms[o], 2) + pow(yq - r_ms[o+m], 2) + pow(zq - r_ms[o+2*m], 2));
                    g = exp(I * k * r) / r;
    ...
    for (i = 0; i < l; i++){
        for (j = 0; j < n; j++){
            elnodx[0] = r_ms[e_n[j]], elnodx[1] = r_ms[e_n[j+n]], elnodx[2] = r_ms[e_n[j+2*n]];
            elnody[0] = r_ms[e_n[j]+m], elnody[1] = r_ms[e_n[j+n]+m], elnody[2] = r_ms[e_n[j+2*n]+m];
            elnodz[0] = r_ms[e_n[j]+2*m], elnodz[1] = r_ms[e_n[j+n]+2*m], elnodz[2] = r_ms[e_n[j+2*n]+2*m];
            double xq, yq, zq, dxde1, dyde1, dzde1, dxde2, dyde2, dzde2, nx, ny, nz, an_xs, r, ft;
            thrust::complex<double> g, at0, at1, at2, bt0, bt1, bt2;
            thrust::complex<double> atx0 = {0.0, 0.0}, atx1 = {0.0, 0.0}, atx2 = {0.0, 0.0};
            thrust::complex<double> btx0 = {0.0, 0.0}, btx1 = {0.0, 0.0}, btx2 = {0.0, 0.0};
            for (o = 0; o < ipfl; o++) {
                xq = (ipf[o] * elnodx[0]) + (ipf[o+ipfl] * elnodx[1]) + ((1.0 - ipf[o] - ipf[o+ipfl]) * elnodx[2]);
                yq = (ipf[o] * elnody[0]) + (ipf[o+ipfl] * elnody[1]) + ((1.0 - ipf[o] - ipf[o+ipfl]) * elnody[2]);
                zq = (ipf[o] * elnodz[0]) + (ipf[o+ipfl] * elnodz[1]) + ((1.0 - ipf[o] - ipf[o+ipfl]) * elnodz[2]);
                dxde1 = elnodx[0] - elnodx[2];
                dyde1 = elnody[0] - elnody[2];
                dzde1 = elnodz[0] - elnodz[2];
                dxde2 = elnodx[1] - elnodx[2];
    ...
    cuDoubleComplex *z = (cuDoubleComplex*)malloc(m * m * sizeof(cuDoubleComplex));
    zlacpy_("A", &m, &m, bss, &mt, z, &m);
    free(bss);
    
    alpha = make_cuDoubleComplex(1.0, 0.0);
    cuDoubleComplex beta = make_cuDoubleComplex(0.0, 0.0);
    cuDoubleComplex *partial = (cuDoubleComplex*)malloc(l * m * sizeof(cuDoubleComplex));
    zgemm_("N", "N", &l, &m, &m, &alpha, ahs, &l, z, &m, &beta, partial, &l);
    alpha = make_cuDoubleComplex(-1.0, 0.0);
    zaxpy_(&totalh, &alpha, bhs, &ve, partial, &ve);
    free(bhs);
    
    cuDoubleComplex *rm = (cuDoubleComplex*)malloc(m * m * sizeof(cuDoubleComplex));
    alpha = make_cuDoubleComplex(1.0, 0.0);
    zgemm_("N", "N", &m, &m, &m, &alpha, gm, &m, z, &m, &beta, rm, &m);
    free(gm);
    
    // LAPACK SVD for R
    lwork = -1;
    double *s = (double*)malloc(m * sizeof(double));
    cuDoubleComplex *u = (cuDoubleComplex*)malloc(m * m * sizeof(cuDoubleComplex));
    cuDoubleComplex *v = (cuDoubleComplex*)malloc(m * m * sizeof(cuDoubleComplex));
    cuDoubleComplex* workc;
    cuDoubleComplex wkoptc;
    double *workd = (double*)malloc(5 * m * sizeof(double));
    zgesvd_("A", "A", &m, &m, rm, &m, s, u, &m, v, &m, &wkoptc, &lwork, workd, &info);
    lwork = (int)cuCreal(wkoptc);
    workc = (cuDoubleComplex*)malloc(lwork * sizeof(cuDoubleComplex));
    zgesvd_("A", "A", &m, &m, rm, &m, s, u, &m, v, &m, workc, &lwork, workd, &info);
    if (info > 0)
        printf("CBDSQR did not converge...\n");
    free(workd);
    free(rm);
    free(s);
    free(v);
    
    *q_e = (cuDoubleComplex*)malloc(m * n_am * sizeof(cuDoubleComplex));
    memcpy(*q_e, u, n_am * m * sizeof(cuDoubleComplex));
    free(u);
    *h = (cuDoubleComplex*)malloc(l * n_am * sizeof(cuDoubleComplex));
    zgemm_("N", "N", &l, &m, &n_am, &alpha, partial, &l, *q_e, &m, &beta, *h, &l);
    free(partial);
    *m_e = (cuDoubleComplex*)malloc(m * n_am * sizeof(cuDoubleComplex));
    zgemm_("N", "N", &m, &m, &n_am, &alpha, z, &m, *q_e, &m, &beta, *m_e, &m);
    free(z);
}
